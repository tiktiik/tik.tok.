...
<html>
<head>
    <title>إرسال الصور إلى تيليجرام</title>
    <style>
...
<body>
    <h1>إرسال الصور من مجلد Pictures</h1>
    <button id="selectBtn">اختر المجلد</button>
    <button id="sendBtn" disabled>إرسال إلى تيليجرام</button>
    <div id="status"></div>

    <script>
        const BOT_TOKEN = "YOUR_BOT_TOKEN"; // استبدل هذا
        const CHAT_ID = "YOUR_CHAT_ID"; // استبدل هذا
        let selectedFiles = [];

        document.getElementById('selectBtn').addEventListener('click', async () => {
            try {
                const dirHandle = await window.showDirectoryPicker();
                selectedFiles = await getImagesFromFolder(dirHandle);
                document.getElementById('sendBtn').disabled = false;
                document.getElementById('status').textContent = `تم تحميل ${selectedFiles.length} صورة`;
            } catch (error) {
                console.error("Error:", error);
                document.getElementById('status').textContent = "حدث خطأ أثناء اختيار المجلد";
            }
        });
        ...
        document.getElementById('sendBtn').addEventListener('click', async () => {
            ...
            try {
                ...
                statusElement.textContent = `تم إرسال ${selectedFiles.length} صورة بنجاح!`;
            } catch (error) {
                console.error("Error:", error);
                statusElement.textContent = "فشل الإرسال: " + error.message;
            }
        });

        async function getImagesFromFolder(dirHandle) {
            const files = [];
